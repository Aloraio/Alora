...
There are three parties in this setup: the alora node, the alora client, and a third party client.
The alora client serves the purpose of taking in requests from the third party client and relaying them
to an arbitrary alora node.

```
Your Client <---> alora Client <---> alora Node
                   Full API            Full API
                   + Prove
                   + RunNock
```

The API of the alora Client is the same as the one of the alora Node, save for the `prove` and
...
iex(1)> node = alora.Node.Examples.ENode.start_node()
...
%alora.Node.Examples.ENode{
  grpc_port: 63796,
  tcp_ports: [],
  pid: #PID<0.345.0>,
  node_id: "62831092"
}
iex(3)> {:ok, client} = alora.Client.connect("localhost", node.grpc_port, 50051, node.node_id)
{:ok, %alora.Client{grpc_port: 50051, supervisor: #PID<0.396.0>, type: :grpc}}
```

### Using a binary

You build the binary from within the root directory of the alora project.

```text
~/Documents/Work/anoma  (testnet-01) % mix do --app anoma_client escript.build
==> alora_client
Generated escript alora_client with MIX_ENV=dev
```

The binary can be started if you know the hostname and port of the alora node (see above).
...
```text
~/Documents/Work/alora  (testnet-01) % ./apps/alora_client/alora_client --listen-port 50051 --node-host localhost --node-port 63796 --node-id 62831092
...
The proto file (apps/alora_protobuf/priv/protobuf/anoma.proto) defines the structure and data types
